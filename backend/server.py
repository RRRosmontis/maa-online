...
GAME_CODE = os.getenv("MAA_ONLINE_GAME_CODE", "mrfz")
TOKEN_FILE = Path(os.getenv("MAA_ONLINE_TOKEN_FILE", str(BASE_DIR / "token")))
HOST = os.getenv("MAA_ONLINE_HOST", "127.0.0.1")
PORT = int(os.getenv("MAA_ONLINE_PORT", "22888"))
WIDTH = int(os.getenv("MAA_ONLINE_WIDTH", "1280"))
...
class VideoSnapshotter:
        ...
        try:
            while self._running:
                ...
                if frame.width > 0 and frame.height > 0:
                    app_state.width = frame.width
                    app_state.height = frame.height
    ...
    async def snapshot_bytes(self, fmt="png") -> Optional[bytes]:
        if not self._got_first.is_set():
            return None
        if self._last_recv_ts and time.monotonic() - self._last_recv_ts > 10:
            self._last_frame = None
        ...
        try:
            ...
            logging.exception("Failed to encode cloud-game frame: %s", exc)
            return None
...
async def handle_info(request: web.Request):
    if not app_state.is_ready:
        ...
        return web.json_response({
            "status": status,
            "message": app_state.last_error or "Cloud gaming service not ready or not connected.",
        })
...
async def handle_account(request: web.Request):
    if not app_state.token:
        return web.json_response({"status": "login_required"}, status=401)
    loop = asyncio.get_running_loop()
    user_info = await loop.run_in_executor(None, fetch_user_info, app_state.token)
    if not user_info:
        return web.json_response({"status": "error", "message": "Failed to query account."}, status=502)
    return web.json_response({
        "status": "ok",
        "remaining_time": user_info.get("free_time_left"),
    })
...
async def handle_screencap(request: web.Request):
    ...
    image_bytes = await app_state.snapshotter.snapshot_bytes(fmt="png")
    if not image_bytes:
...
async def handle_click(request: web.Request):
    ...
    if not (0 <= x < app_state.width and 0 <= y < app_state.height):
        return web.json_response({"status": "error", "message": "Coordinates out of range"}, status=400)
...
async def handle_swipe(request: web.Request):
    ...
    try:
        ...
        return web.json_response({"status": "error", "message": "Invalid request body"}, status=400)
    if not (50 <= swipe_duration <= 10000):
        return web.json_response({"status": "error", "message": "Duration must be between 50 and 10000 ms"}, status=400)

    logging.warning(f"Action: Swipe from ({start_x}, {start_y}) to ({end_x}, {end_y}) in {swipe_duration}ms")

    # The web client protocol uses raw video-frame pixel coordinates, matching
    # the existing mouse move/click commands. Keep swipe in the same space.
    def clamp_coord(pixel_val: int, dimension: int) -> int:
    ...
    def create_touch_cmd(evt_type: int, px: int, py: int, tid: int) -> dict:
        """Create a touch command in raw video-frame pixel coordinates."""
        raw_x = clamp_coord(px, screen_width)
        raw_y = clamp_coord(py, screen_height)
        timestamp = str(int(time.time() * 1000))
        cmd_str = f"{evt_type} {raw_x} {raw_y} {tid}"
...
async def handle_input(request: web.Request):
    ...
    if not text or len(text) > 1024:
        return web.json_response({"status": "error", "message": "Text length must be between 1 and 1024"}, status=400)
...
async def handle_start(request: web.Request):
    ...
    if not TOKEN_FILE.is_file():
        return web.json_response(
            {"status": "login_required", "message": "Run login.py before starting the cloud game."},
            status=401,
        )
    ...
    if not app_state.cloud_game_task:
        app_state.last_error = None
        app_state.ready_event.clear()
        app_state.cloud_game_task = asyncio.create_task(run_cloud_game())

    try:
        await asyncio.wait_for(app_state.ready_event.wait(), timeout=START_WAIT_SECONDS)
    except asyncio.TimeoutError:
        return web.json_response(
            {"status": "connecting", "message": "Cloud game connection is still starting."},
            status=202,
        )

    if app_state.is_ready:
        return web.json_response({"status": "ok", "message": "Cloud game connection is ready."})

    return web.json_response(
        {"status": "error", "message": app_state.last_error or "Cloud game connection failed."},
        status=502,
    )
...
async def run_cloud_game():
    try:
        try:
            ...
            token = ""
        if not token:
            raise RuntimeError("login token is missing; run login.py first")

        app_state.token = token
        print(f"{Colors.CYAN}[*] Connecting to cloud gaming service...{Colors.RESET}")
        res, sock = await connect(token, GAME_CODE, w=app_state.width, h=app_state.height, fps=FPS)
        app_state.sock = sock
        ...
        await asyncio.wait_for(track_event.wait(), timeout=20.0)
        ...
        if token_to_use:
            ...
            try:
                ...
                logging.warning("Failed to release remote cloud-game session: %s", exc)
...
async def run_server():
    ...
    app.add_routes([
        ...
        web.get('/account', handle_account),
